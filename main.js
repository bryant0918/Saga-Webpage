...
function validateField(field) {
    ...
    if (field.name === "generations") {
        ...
        if (
            value &&
            (isNaN(generations) || generations < 2 || generations > 10)
        ) {
            isValid = false;
...
async function exchangeCodeForTokenAndRedirect(code) {
    ...
    const returnedState = new URLSearchParams(window.location.search).get(
        "state",
    );
    ...
    try {
        ...
        const tokenResponse = await window.appConfig.exchangeToken(
            code,
            returnedState,
        );
