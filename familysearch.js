...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function generateFamilyTree() {
        ...
        const startingPerson = document.getElementById('startingPerson').value.trim();
        const familyName = document.getElementById('familyName').value.trim();
        ...
        try {
            ...
            const currentPerson = await window.fetchCurrentPerson(currentAccessToken);
            const currentPersonName = currentPerson ? currentPerson.name : 'Unknown';
    ...
    function showRequestSubmitted() {
        if (successMessage) {
            ...
            if (successText) successText.textContent = 'Thanks for submitting a family tree request. We will contact you shortly.';
    ...
    async function fetchCurrentPerson(accessToken) {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch current person: ${response.status}`);
            }
