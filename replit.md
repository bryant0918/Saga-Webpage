...
- `dashboard.js` — Dashboard-specific logic: fetches user profile from FamilySearch API, loads tree data from `family-trees.replit.app` endpoints (`/people/tree/kids`, `/people/tree/husb`, `/people/tree/wife`), renders expandable person list sorted by children → husband's ancestors → wife's ancestors. Person images loaded via `POST /people/tree/image`. Inline editing of names/dates via pencil icons (`POST /people/tree/update`) and image replacement via click-to-upload (`POST /people/tree/update-image`).
